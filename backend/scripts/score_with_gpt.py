...
def main():
    ...
    for idx, image_path in enumerate(original_student_images):
        try:
            if not os.path.exists(image_path):
                print(f"❌ student_image[{idx}] path not found: {image_path}", file=sys.stderr)
                log_stderr(f"❌ student_image[{idx}] path not found: {image_path}")
                continue
    ...
    try:
        ...
        question_image_analysis = result.get("questionImageAnalysis", "N/A")
        student_image_analysis = result.get("studentImageAnalysis", [])
        print(json.dumps({
            "score": score,
            "matched": matched,
            "reason": reason,
            "questionImageAnalysis": question_image_analysis,
            "studentImageAnalysis": student_image_analysis
        }))
    except Exception as e:
